#include "constants.h"
#include "types.h"  // ✅ MUST be included before display or any game logic
...
bool sound = false;  // or false if you want it off by default
...
// Shoot
void fire() {
  playSound(600, 100);  // 600 Hz for 100 ms
    ...
    switch (type) {
      ...
      case E_MEDIKIT: {
          if (entity[i].distance < ITEM_COLLIDER_DIST) {
            // pickup
            playSound(500, 80);  // 600 Hz for 100 ms
...
void updateHud() {
  display.fillRect(12, 58, 15, 6, BLACK);
  display.fillRect(50, 58, 5, 6, BLACK);
...
// Debug stats
void renderStats() {
  display.fillRect(58, 58, 70, 6, BLACK);
